...
pub fn resolve_manifest_dependencies(manifest: &Manifest) -> Result<ResolvedDependencyTree, Box<dyn Error>> {
    let home_dir = env::home_dir().ok_or_else(|| Box::new(CliError::from("Could not find home directory")))?;
    ...
    for (name, version) in manifest.dependencies.iter() {
        ...
        for package_dir in package_dirs.iter() {
            let manifest_path = package_dir.join(name).join(version).join(MANIFEST_FILE_NAME);
