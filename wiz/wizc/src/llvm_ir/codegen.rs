...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn binop(&mut self, b: MLBinOp) -> AnyValueEnum<'ctx> {
        ...
        match (lft, rit) {
            ...
            (AnyValueEnum::PointerValue(left), AnyValueEnum::PointerValue(right)) => match b.kind {
                ...
                MLBinOpKind::Equal => {
                    let pint = self.context.ptr_sized_int_type(self.execution_engine.get_target_data(), None);
                    let left = self.builder.build_ptr_to_int(left, pint, "");
                    let right = self.builder.build_ptr_to_int(right, pint, "");
                    let v = self.builder.build_int_compare(IntPredicate::EQ, left, right, "peq");
                    v.as_any_value_enum()
                },
                ...
                MLBinOpKind::NotEqual => {
                    ...
                    let left = self.builder.build_ptr_to_int(left, pint, "");
                    let right = self.builder.build_ptr_to_int(right, pint, "");
                    let v = self.builder.build_int_compare(IntPredicate::NE, left, right, "pnq");
                    v.as_any_value_enum()
                },
    ...
    fn ml_type_to_type(&self, ml_type: MLValueType) -> AnyTypeEnum<'ctx> {
        match ml_type {
            MLValueType::Primitive(name) => match name {
                MLPrimitiveType::Unit | MLPrimitiveType::Noting => {
                    AnyTypeEnum::from(self.context.void_type())
                }
